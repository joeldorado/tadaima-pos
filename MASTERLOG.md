...
horneaba `VITE_API_URL=https://tadaima-747134907044.us-central1.run.app`, el
servicio que se borró al mudar el POS a us-east1. Todas las llamadas de la
tienda (catálogo, login, checkout) daban 404. Fix: el ARG ahora es el dominio
`https://tadaimamexico.com` (CORS ya permitía tadaimausa.com; verificado 200 +
preflight 204). Redeploy del servicio `tadaimaus` (proyecto `tadaimaus`,
us-central1) → rev `tadaimaus-00004-z4c` al 100%. Verificado en navegador: el
bundle nuevo llama a `tadaimamexico.com/api/v1/us/catalog` (200) y pinta 55
productos.
**Rollback:** `tadaimaus-00003-77l` (pero apunta al host muerto — no sirve).

**Misma causa en la app móvil** (repo `tadaima-app-pos`): `eas.json` apuntaba
al mismo host muerto → fix `a0b9c19` en `desarrollo` + PR
joeldorado/tadaima-app-pos#1 (desarrollo → main, sin merge). Lección grabada en
AGENTS.md: **usar siempre el dominio, nunca un `*.run.app`**.

**AGENTS.md en la raíz** (lo leen solos Claude, Codex, Cursor, Copilot, Gemini):
qué es, estado de prod, reglas de oro de operación y reglas de negocio
verificadas contra el código. `CLAUDE.md` lo importa (`@AGENTS.md`) y ya no dice
MySQL. `backend/AGENTS.md`: fuera la credencial del admin de prod (repo
PÚBLICO), fuera "QA directo contra prod" y el proxy de Cloud SQL; Laravel 13.
MASTERLOG: contraseña de Pier tachada en 4 líneas. ⚠️ Sigue en el historial de
git → **hay que cambiar la contraseña de Pier** (y el default de
`PierFreshSeeder`).

**Hallazgo de infraestructura:** el servicio `tadaima` en us-east1 tiene
`traffic: latestRevision=true` — ya NO ancla tráfico ni tiene tag `ruben`
(eso era del servicio viejo). Un `gcloud run deploy` queda vivo al 100% de
inmediato; el flujo seguro documentado es `--no-traffic --tag candidate` →
smoke → `update-traffic --to-latest`.
...
| # | Área | Feature | Sesión |
|---|------|---------|--------|
| 154 | Caja | **El ticket sale directo, sin preguntar. DEPLOYADO rev `tadaima-00140-29m`** (bundle `index-UG8kbDdd.js`). El cliente pidió quitar el modal "¿Imprimir ticket?" que salía al cobrar: que el ticket se mande a imprimir solo. **Desactivado comentando, no borrando** — el JSX del modal (~L7500) queda intacto e inalcanzable, listo para revivir descomentando el bloque de `triggerPrintFlow`. **La trampa:** comentar únicamente `setShowPrintModal(true)` habría roto las ventas paralelas — `closePendingMesa()` (el que cierra Venta 2..5 tras cobrar) **vivía solo en los botones del modal**, así que sin modal esas mesas se quedarían abiertas para siempre. Por eso el flujo cae al camino de impresión directa, que sí lo llama. **Se ignora `tadaima_print_pref` a propósito:** un cajero que alguna vez marcó "no preguntar de nuevo" + OMITIR tiene `'never'` guardado y **hoy no imprime nada**; respetarlo dejaría esa caja sin tickets justo cuando el cliente los pidió. Se arregla en un solo lugar porque las 3 ramas de cobro (venta regular, liquidación de folio, anticipo mixto) pasan todas por `triggerPrintFlow`. **QA en navegador, contra el backend local:** sin modal; **una** ventana de ticket y **un** `print()` por venta (instrumentando `window.open` y `print` — se descartó el doble ticket); con `pref="never"` puesto a mano **igual imprime**; y cobrando desde una Venta 2 la mesa **se cierra sola** (`[CAJA PRINCIPAL]` → `+` → `[CAJA PRINCIPAL, VENTA 2]` → cobrar → `[CAJA PRINCIPAL]`). Cazado de paso en el QA: `addMesa` se niega a abrir mesa si la actual está vacía (por eso el primer intento del test no creaba la Venta 2 — era el test, no el código). Verificado en el bundle vivo: las únicas 2 lecturas de `tadaima_print_pref` que sobreviven están **dentro del JSX del modal** (la condición del link "Restablecer preferencia"), no en el flujo. vitest 114/114 · build OK · tsc en baseline 470 | 2026-07-20 |
| 153 | Caja | **El carrito ya no se queda con promos y precios muertos. DEPLOYADO rev `tadaima-00139-xtc`** (bundle `index-CEru5bqj.js`; verificado en vivo: "Cambiaron las promociones de", `cart-sync-promos`/`cart-sync-flags`, "El precio de", "fuera del catálogo", `ids:n,active:!0` en el bundle, y `GET /products?ids=1` → 401 = la ruta existe). Joel borró una promo 2x1 y creó una de mayoreo; **Caja siguió mostrando y cobrando con la borrada**. No era el cache de React Query (la invalidación del tab de Promos sí corre): **cada línea del carrito guarda una COPIA del producto** —con `active_promotions` adentro— persistida a localStorage por zustand, y **nada la refrescaba jamás**; la hidratación al montar solo renombra mesas y rellena `lineId`. **El bug real no es cosmético, es un callejón sin salida:** al cobrar el server recomputa con las promos vivas y rechaza (`"Los pagos (150) no coinciden con el total (200)"`), el handler invalida productos y pide "revisa el total y cobra de nuevo" — **pero el total nunca cambia porque la línea sigue congelada**. Única salida: borrar la línea y re-agregarla. **El mismo callejón existía con precios y era peor**: `assertPricesMatchCatalog` rechaza con "Precio $X fuera del catálogo" y `handleCheckoutError` **no tenía rama para ese mensaje** → toast genérico, sin refetch ni explicación. Decisión de Joel: la línea toma el precio nuevo sola, con aviso. **`lib/cartSync.ts` (nuevo, puro, 29 tests):** sincroniza promos, flags de pago y `price_a..e`. **Ausencia en el índice = no-op** (el pool encoge cuando el cajero borra la búsqueda: vaciar promas por "no lo encontré" sería peor que el bug). **`promoSignature` compara la MATEMÁTICA, no los ids** — una promo se edita conservando su id (−$100 → −$50) y comparando sets de ids ese cambio pasaría en silencio. Devuelve **la referencia de entrada** si nada cambió → bailout de React. **Excluye** líneas de preventa (`price_a` prorrateado / entregadas en $0 — resucitarles el precio de catálogo sería el peor caso) y el precio de las dañadas (lo gobierna `damagedPrice` y el server salta el guard); **incluye** los splits (padre e hija comparten `product.id`; excluir una las desincroniza contra el zip posicional del server). `active` NO se sincroniza: es **inobservable**, los 3 hooks mandan `active:true` y el controller filtra, así que un producto desactivado desaparece del payload y nunca llega como `false`. `stock` tampoco: churn constante y ya tiene su propia ruta de recuperación. **`?ids=` en `ProductController`:** el pool de Caja es `sort=top` capado a 200 — un ORDER BY, **no un filtro** — así que un producto de poca venta caía fuera y su snapshot no tenía contra qué compararse; sin esto el arreglo funcionaba "solo para los populares", justo lo que Joel no podía verificar con su producto de prueba. Hereda el `forStore()` del embed; **no se usó `ProductController::show`**, que carga `activePromotions` SIN scoping y traería promos de otras sucursales. **`lib/productAdapter.ts` (nuevo):** un solo `ProductLight → Product` con tipo de retorno anotado y **cero cast**. Los tres `as Product` que había son exactamente lo que dejó pasar campos faltantes en silencio: la ruta de búsqueda perdía `active_promotions` y **la de escaneo además `allow_cash`/`allow_card`/`product_type`** → un producto solo-efectivo escaneado fuera del top 200 **no disparaba el bloqueo de Caja** (el chequeo es `=== false` y llegaba `undefined`) y el rechazo aparecía hasta el checkout: **regresión viva de la restricción de pago de la rev 00138**. Quitar el cast destapó de inmediato un error real de `exactOptionalPropertyTypes` (resuelto con spreads condicionales, no con otro cast). **Bug de QA cazado en el navegador:** el aviso salía **3 veces** porque el efecto leía `mesasRef`, que se actualiza un commit más tarde, y las 3 queries que alimentan el índice resuelven en momentos distintos → las corridas 2 y 3 veían el carrito viejo. Fix: depender de `mesas` (el estado real) — no hay bucle porque la corrida extra ya no encuentra nada que cambiar — más ids fijos de toast. **`PromosPage`** usaba `["promos-products"]`, fuera del namespace `['products']`, así que ninguna invalidación la alcanzaba (dependía de su interval de 60s); el sufijo `'global'` de la key nueva evita colisionar con las de Caja, que sí van store-scoped (le daría promos de otras sucursales). **QA en navegador, flujo real por la UI (no por API):** $150 → **$200** solo, badge "Buen Fin 2026 · −$100", **un** aviso, y **cobro aceptado al primer intento**; el total viejo ($150) el server lo rechaza — que era el bucle. 40s moviendo cantidades: cero avisos, total quieto. Precio $150→$175: la línea lo toma con aviso ámbar y cobra. Tests **335/335** (+4) · vitest **114/114** (+29) · type-check en baseline (470, sin errores nuevos). **Hueco conocido:** editando desde OTRA máquina, Caja tarda hasta 60s o hasta que se enfoque la ventana; desde la misma es inmediato | 2026-07-19 |
| 152 | Promos / Caja | **Restricción de método de pago POR PROMO + fin del bloqueo mudo. DEPLOYADO rev `tadaima-00138-wz6`** (bundle `index-BaS4_8TY.js`; verificado en vivo: las 2 promos vivas convertidas a −$100 c/u — Buen Fin 2 pzas = −$200, Promo 3x2 3 pzas = −$300 —, `tiers` ya no viaja en el payload, y los marcadores de UI presentes). **Este deploy trajo también las migraciones del mayoreo (#151), que nunca habían corrido en prod.** Le piden a Joel que una promo pueda ser solo-efectivo o solo-tarjeta. Decisión suya: **BLOQUEA el cobro** igual que hoy lo hace la restricción del producto (no "se cae el descuento"), con aviso claro, y con una salida: **"Cobrar sin promo"** por línea. **Esa decisión evitó rehacer los motores**: como el bloqueo es *consumidor* del cálculo y nada retroalimenta el monto, NO hubo que meterle el método de pago a `recalculateSale`/`calculate`, ni canal de "promo rechazada", ni columna en `sale_items`, ni reordenar `CheckoutService` (con la alternativa descartada sí habría hecho falta todo eso). **Datos:** `allow_cash`/`allow_card` en `product_promotions`, espejo de `product_payment_methods`. **Guard backend:** `$calc` es local de `checkoutDirect()` y el guard corre en `checkout()` — otro método; lo que cruza es `$v2Lines`, que ya lleva `applied_promotion_id`, así que se **enriquece con los flags de la promo donde `$activePromos` ya está en memoria** (0 queries) y `assertPaymentMethodsAllowed` gana índice posicional. **Solo bloquea si la promo REALMENTE aplicó** (`applied_promotion_id != null`): la restricción es propiedad del beneficio otorgado, no del producto — con 1 pieza y un 2x1 que no disparó, no hay nada que restringir. `$v2Lines` es null en la ruta clásica (calc_version != 2) y ahí no restringir es correcto. **Validación** `promoPaymentConflictError()` en el CONTROLLER, no en el FormRequest, porque tiene que re-correr en cada update (los flags del producto cambian después de crear la promo): promo solo-efectivo sobre producto que no acepta efectivo = invendible, y ambos flags en false = nunca cobrable. La regla `boolean` no es cosmética: sin ella un `"false"` string llega a Eloquent, `(bool)"false" === true` y la restricción se guarda AL REVÉS. **FIN DEL BLOQUEO MUDO (lo de más valor):** el botón Cobrar se apagaba gris diciendo "Cobrar" sin decir por qué, y el único mensaje que lo explicaba vivía en el `onClick` — **inalcanzable, porque un botón `disabled` no dispara clic**. Ahora: label `"No se puede con {método}"` + `title`; **banner fijo SIN X** (el `customerLimitWarning` lleva X porque es un evento pasado; esto es una condición viva y cerrarla devolvería el estado mudo) que nombra al culpable y da la salida; `title` en las opciones deshabilitadas del menú (antes solo un triángulo de 11px); botón "Separar venta" **siempre visible** con bloqueo (antes se escondía justo en el carrito mixto = Caja muerta); auto-switch **simétrico y nombrando el producto** (la rama de escaneo cambiaba el método EN SILENCIO, y solo-tarjeta no disparaba nada en ninguna — importa porque cambiar de método dispara `repriceForSocio`, o sea mueve precios). **Simplificaciones:** se borró `payment_restriction` (string derivado, menos expresivo que los flags — no podía representar solo-tarjeta — y causa raíz del auto-switch asimétrico; queda solo como normalización en la frontera legacy); el `items.some(!itemAcceptsMethod)` que se recalculaba en 4 sitios con 4 copys distintos se colapsó en **un memo `blockedItems`**; los 2 badges ad-hoc por línea (uno ni cubría Transferencia) se fusionaron en uno derivado que además distingue producto de promo. Tests **331/331** (+9) · vitest 85/85 · QA en navegador: botón "NO SE PUEDE CON TARJETA", banner nombrando la promo, tooltip correcto en la opción deshabilitada, y "Cobrar sin promo" dejando el total en $450 exacto | 2026-07-18 |
| 151 | Promos | **Mayoreo — reemplaza "Descuento por cantidad". DEPLOYADO junto con #152 en rev `tadaima-00138-wz6`**. Joel pidió "a partir de N piezas, −$X a CADA una" (5 pzas = −$500, 7 = −$700, 10 = −$1,000). El tipo `qty_discount` existente hacía algo DISTINTO: cobraba **por grupo** (`tiers=[{qty:2,amount:100}]` = −$100 por cada par, y las piezas sueltas no recibían nada → 5 pzas daban −$200). Decisión de Joel: **reemplazar** (no un tercer tipo) y **un solo escalón**, sin escalera. **Se conserva el slug `qty_discount`** — cambió la matemática, no el identificador — para no migrar el `type` de las filas vivas ni tocar las uniones TS duplicadas en 4 archivos. **Datos:** columnas `min_qty` (smallint) + `discount_per_unit` (decimal 10,2 como todo el dinero; se descartó (10,4) porque divorciaría la etiqueta del cobro), en 2 migraciones separadas (esquema + backfill, precedente `2026_06_03_000001`). El backfill toma el `amount` viejo **TAL CUAL como descuento por pieza** (`per_unit = amount`, corregido 2026-07-23 tras revisión de Joel: él siempre leyó "2 → −$100" como −$100 a CADA una). Sus promas vivas por tanto descuentan MÁS que con el modelo por grupos — Buen Fin 2 pzas: $100 → **$200**; Promo 3x2 3 pzas: $100 → **$300** — y eso es lo buscado. La primera versión dividía (`amount/qty`) para "conservar el total"; se descartó porque además de no ser lo pedido, **ni siquiera lo conservaba**: 100 ÷ 3 = 33.33 y 3 pzas daban $99.99. Las **multi-escalón se PAUSAN y se loguean** en vez de convertirse mal (tomar el escalón menor parte a la mitad el descuento del mayor; tomar el mayor lo dispara — no hay conversión honesta). Idempotente por `whereNull('min_qty')`, no solo por la tabla migrations. **`tiers` NO se dropea de la DB** (hace real el `down()` y es el único rastro para reconfigurar las pausadas) pero **sí se quita del payload** de los 3 resources — dejarlo sería renderizar un precio con la matemática vieja. **Motor** (gemelos PHP/TS): `mayoreoAmount(minQty, perUnit, qty)` = `floor(qty) × perUnit` si `floor(qty) >= minQty`; `floor` en umbral Y multiplicador (la API acepta fraccionarios, media pieza no gana descuento), clamp al bruto con la qty REAL. **Dos bloqueadores cazados en la planeación:** (1) el `only(['name','buy_n','pay_m','tiers','priority'])` del controller (L65 y L109) habría dejado las columnas nuevas en NULL — promo que existe, se ve y **nunca descuenta, en silencio**; (2) `tiers=>prohibited` + `min_qty=>required` rompían pausar/reanudar desde un bundle rezagado (`toggleStatus` reenvía el objeto completo) → **presencia condicional por método** (`required` al crear, `sometimes` al actualizar) y `tiers` aceptado-e-ignorado. Se destapó además un bug latente: `update()` usaba `$request->promoType()` cuyo default es 'nxm', así que un PUT parcial comparaba contra el tipo equivocado. **UI:** el repetidor de escalones (con su tope de 5, `parsedTiers`, botón "Agregar escalón") se fue; quedan 2 inputs + ejemplo calculado en vivo. Pill roja **"Sin configurar"** con toggle deshabilitado para las que pausó el backfill (no hay pantalla de edición: se borran y se rehacen). `promoLabel` ahora dice **"3+ pzas −$50 c/u"** — el "c/u" corrige una deshonestidad viva: decía "2+ pzas −$100" cuando eran $100 por PAR. **Bug vivo arreglado:** `PromosPage` hardcodeaba `${buy_n}x${pay_m}` en 3 lugares (nombre del PNG, título del share y subtítulo del modal) → con las 2 promos `qty_discount` de prod imprimía literalmente **"Promo nullxnull"**; se colapsó también `promoDisplay`, la 4ª copia paralela de la lógica de etiquetas. Tests **322/322** (+9, incl. la migración: single→convertida, multi→pausada, idempotencia) · vitest 84/84 · QA en navegador: Caja mostró **$150 / $300 / $700** para 1/3/7 pzas y el pill "Promo 3+ pzas −$50 c/u". Riesgo de bundle viejo re-calibrado: el SW **no cachea el bundle** (`globPatterns: []`) y el server rechaza con 422 si el pago no cuadra con su recálculo → nunca cobra mal, a lo más "no me deja cobrar" durante el deploy | 2026-07-18 |
| 150 | Tienda Online | **Catálogo v5 — top manual arrastrable + previews de apariencia. DEPLOYADO rev `tadaima-00137-k94`** (bundle `index-C-H-ob7j.js`; verificado en vivo: `x-frame-options: SAMEORIGIN`, `PUT /catalog/featured-order` → 401 (existe), `catalog_position` en el payload público, los 5 marcadores de UI en el bundle, y `GalaxyBackground-D18vl-xx.js` 55.8 kB servido aparte con 0 ocurrencias de `galaxyfield` en el bundle principal). (1) **Top manual**: `featured` solo decía SI un producto sale primero; entre destacados mandaba `id DESC` y el admin no elegía el orden. Nueva columna `products.catalog_position` (nullable, **sin índice** a propósito: el ORDER BY va sobre la expresión `(catalog_position IS NULL)`, que MySQL no resuelve por índice, y el `whereExists` de stock ya impide el escaneo ordenado). Helper `applyCatalogOrder()` compartido por `publicCatalogGlobal` y `productFlags` → `orderByRaw('(products.catalog_position is null) asc')` + posición + featured + `id DESC` de cierre; el truco `IS NULL` es el portable (**`NULLS LAST` no existe en MySQL 8**) y el `id DESC` final garantiza orden TOTAL, que es lo que necesita el "Cargar más" acumulativo para no duplicar ni perder items. Endpoint `PUT /catalog/featured-order` con body `{order:[ids]}` (posición = índice), en **ruta aparte** de `product-flags/` porque `product-flags/reorder` chocaba con el binding implícito de `{product}`; filtra ids que ya perdieron la ★ y **devuelve la lista canónica** (el front reemplaza su estado con ella — toda la reconciliación necesaria para 2 pestañas, sin locks). Quitar la ★ limpia la posición (`wasChanged('featured')` + `forceFill`); `catalog_position` **NO va en `$fillable`** para que ningún `update($request->validated())` futuro la toque. (2) **UI**: botón "Ordenar" en el panel Productos → modo aparte con `Reorder.Group/Item` de **`motion/react`** (ya instalado y ya en el bundle del admin — **cero dependencias nuevas**; se desinstalaron `react-dnd`/`react-dnd-html5-backend`, huérfanas con cero imports). Guardado optimista en `onDragEnd` (NO en `onReorder`, que dispara decenas de veces durante el arrastre) con rollback al último orden confirmado. Carga **todas** las páginas de destacados antes de habilitar el arrastre: el PUT desacomoda lo no enviado, así que ordenar con media lista cargada borraría posiciones en silencio. Línea divisoria tras el puesto 20 contando solo filas con `in_public_catalog` (campo nuevo: `productFlags` no filtra por active/visible/stock, así que un destacado agotado sale ahí pero no en la tienda). (3) **La tienda abre con el top** aunque `default_sort` sea "Más nuevos" (si no, acomodar no se notaría); se autocorrige porque los acomodados sin stock no llegan en el payload. (4) **Preview antes de publicar**: modal con `<iframe>` a `/catalogo?preview_theme/_bg/_layout` (la página real, no una maqueta) + toggle escritorio/celular 390px. Requirió `docker/nginx.conf` **`X-Frame-Options: DENY` → `SAMEORIGIN`** — **verificado empíricamente contra el build**: con DENY el iframe carga 0 chars (preview en blanco en prod) y con SAMEORIGIN 219k; `vite dev` no manda ese header, así que el bug solo se veía en producción. `createPortal` a `<body>` **obligatorio**: `PanelCard` usa `backdrop-filter` (GLASS) y eso crea bloque contenedor para `fixed` — sin portal el modal se anclaba al panel (bug real encontrado en QA). (5) **Tira 16:9 en vivo** del fondo seleccionado (`ShaderBackground`/`GalaxyBackground` ganan `contained` + `ResizeObserver` sobre `clientWidth` en vez de `window.innerWidth`); una sola tira y no 3 miniaturas vivas porque el shader compone con `min(ancho,alto)` → a 200×56 mostraría un recorte distinto, mentiría igual que el gradiente falso pero gastando GPU. Tests **313/313** (+7) · vitest 83/83 · 0 errores tsc nuevos · chunk `GalaxyBackground-*.js` sigue fuera del bundle principal (verificado por grep) | 2026-07-18 |
| 149 | Tienda Online | **Catálogo v4 — 3 fondos × 3 layouts. DEPLOYADO rev `tadaima-00136-6kl`** (bundle `index-SIwynlDK.js`; API de prod ya expone `appearance.background/layout`; chunk `GalaxyBackground-DhAOIGq9.js` 55.8 kB servido aparte y ausente del bundle principal — verificado por grep). La apariencia pasa de un solo eje (tema) a **tres independientes que se combinan libremente**: color × fondo × diseño. **Fondos** (`catalog_background`): `shader` (la nebulosa de siempre), `gradient` (degradado quieto, nuevo `gradientBg` por tema porque tadaima tenía el `--cat-page-bg` liso) y `galaxy` (**GalaxyField** de la FX library de Joel, teñido con `galaxyColors` por tema). **Layouts** (`catalog_layout`): `classic`, `sidebar` (aside `hidden lg:block` + chips del header `lg:hidden` → en móvil cae al clásico, **una sola rama de render, cero JS de media query**) y `masonry` (CSS `columns`, `imageAspect="natural"` en ProductCard). Compat total: `background` sin configurar hereda `theme.defaultBackground`, así que lo publicado se ve idéntico. **3 huecos de GalaxyField resueltos en el wrapper** (`backgrounds/GalaxyBackground.tsx`): (1) no verifica WebGL y `ogl` deja `gl` en null sin lanzar → probe previo, si falla o hay reduced-motion no monta; (2) escuchaba `pointermove` sin `pointer-events:none` → **se comía los clics de las tarjetas**, ahora wrapper `aria-hidden` + `pointerEvents:none` (se sacrifica el parallax); (3) presupuesto móvil 10k estrellas/dpr 1.5. `React.lazy` obligatorio: `OnlineCatalogPage` es import **eager** en el router, un import estático metía `ogl` (55 kB) en el bundle de todos los cajeros → verificado chunk aparte `GalaxyBackground-*.js` 55.8 kB (17.4 gzip). Extraídos `CategoryList`, `ProductGrid`, `CatalogBody` + `CATALOG_CONTAINER` (el `max-w-5xl` estaba literal en 4 archivos). Admin: Apariencia con 3 pickers y **miniaturas de fondo teñidas con el tema activo** (elegir a ciegas era mala UX) + "Abrir tienda". MCP: `set_background`/`set_layout` (11 tools). Tests **306/306** (+2 whitelist) · vitest 83/83 · 0 errores tsc nuevos (baseline 461 igual con y sin cambios) | 2026-07-18 |
...
| **Admin** | **Pier — `pier@tadaima.mx` / `[contraseña retirada del repo]`** (store_id null, can_view_cost true) |
...
### Sesión 2026-07-18 — Catálogo Online v3 + Reportes de Ruben + PR #2 a main — DEPLOYADO rev tadaima-00135-rzn

Commits `a3b553e` (Catálogo v3) + `e535e84` (merge develop/Ruben) + PR #2 MERGED a main (`a5f25ea`); `develop` fast-forward al mismo punto (Ruben arranca de base completa). Bundle `index-CU3pMZOR.js` verificado (Destacados, temas, cat-social-box, product-flags); API público ya expone `appearance/footer/default_sort/featured` (migración corrió sola). Backend 304/304 (+12 CatalogConfigTest), vitest 83/83.

**(1) Catálogo Online v3 (pedido Joel):** 6 temas de color (`catalogThemes.ts`, contrato vars `--cat-*`, shader con uniform `tint`): tadaima/gradiente-elegante(sin shader)/navidad/halloween/patrio/muertos. Migración `products.featured` + `catalog_visible`; endpoints `GET/PUT /catalog/product-flags` (gate `catalogEditError`, ámbito global); `CatalogConfigService` (KV `catalog_*`: theme/socials JSON/description/show_stores/show_address/show_contact/default_sort, defaults en código, corruptos degradan). Página pública: header slim 2 filas (select nativo de orden, pills unificadas `catalogUi.ts`), pill ★ Destacado (abajo-derecha de la imagen), footer estático (`CatalogFooter`+`SocialLinks` SVG inline: IG/FB/TikTok/X/YT/Discord) con sucursales (address/phone según toggles) y WhatsApp. Fetch público SIEMPRE `sort=featured` (página 1 correcta + paginación estable); "Más nuevos" re-ordena client-side por id. Admin: CatalogTab → shell + 5 sub-tabs en `settings/catalog/` (Visibilidad+orden default, Apariencia con picker de temas, Productos con ★/👁 guardado optimista, Redes, WhatsApp); `CatalogProductManager.tsx` muerto ELIMINADO. Documentación movida del sidebar al menú del avatar. Hardening: llaves `catalog_*` en `PUT /settings` (batch y single) requieren `canEditCatalog` + límite 5000 chars en batch.

**(2) MCP server** `mcp/catalog/` (fuera de workspaces; sdk+zod, fetch puro — `@tadaima/api` es browser-only): 9 tools scoped SOLO a config del catálogo (list_options/get_config/set_theme/set_toggles/set_socials/set_description/set_default_sort/list_products/set_product_flags). `.mcp.json` en la raíz (primero del repo) con `${TADAIMA_API_TOKEN}` del shell. Probado E2E contra local: set_theme navidad→muertos, destacar/ocultar, socials — reflejado en admin UI y catálogo público. OJO: llamadas MCP en un mismo pipe stdio corren CONCURRENTES (get tras set en el mismo batch puede leer viejo).

**(3) Reportes de Ruben (merge develop, 0 conflictos):** `SaleResource.cancelled_items[].cost` (cost_at_sale del snapshot, gateado can_view_cost) — las cancelaciones ahora netean la Utilidad Neta (antes restaban $0 de costo); `GET /supplies/movements` con `from/to/store_id` (mismo `DateRange` día-negocio que /reports/supplies); refactor `exportPdf.ts` (−500 líneas) + exportExcel + tipos; notas "NO QUITAR" en AGENTS.md. QA navegador: Reportes exporta PDF y Excel sin errores. **PENDIENTE de decisión Joel (con cliente):** `docs/PENDIENTE-reportes-efectivo-disponible.md` — tarjeta "efectivo disponible − insumos": qué efectivo (cobrado vs saldo cajón), día vs rango, y no doble-contar insumos origen caja que ya pegan al corte.

**QA pane (aprendizaje):** screenshots del Browser pane salen en blanco con scroll profundo (desync del capturador — verificar por DOM/JS o acortar la página con un filtro); AnimatePresence mode="wait" puede congelar la transición de tabs en el pane (forzar opacity por JS para screenshot) — en prod funciona normal.
...
| Reseed | `php artisan db:seed --class=PierFreshSeeder --force` (`.env` apunta a prod vía proxy). Crea: 1 empresa, 3 roles, 4 métodos de pago, **1 admin Pier** `<pier@tadaima.mx / [contraseña retirada del repo]>`. |
...
- `pos-app` ahora es **repo git independiente** `git@github.com:joeldorado/tadaima-app-pos.git` (separado del monorepo `tadaima-pos`). `.gitignore` ajustado para ignorar `.env`. Login verificado contra prod (pier@tadaima.mx / [contraseña retirada del repo]; el viejo "devaccess" era de la prod pre-reset del 2026-05-30).
