...
|---|------|--------------|---------|
| 117 | Fechas/TZ | **⏳ TODO mañana (Joel) — decidir TZ negocio vs dispositivo** | Hoy (#115) se ancló "hoy" a la TZ del negocio (`America/Mexico_City`) tanto en frontend como en backend (`DateRange`). **Verificar/decidir**: si en el futuro abren una tienda en OTRA zona horaria (otra ciudad/estado), el día se calcularía con la TZ MX fija, no la del lugar. Opciones: (a) TZ por tienda (`stores.timezone`) y que reportes/historial usen la TZ de la tienda activa; (b) dejar MX fijo si todo el negocio opera en hora centro. Joel: "verificar que tome la zona del device, si no creo que tendremos problemas si abren una en otro lugar y toma la del servidor". **Pendiente análisis.** |
