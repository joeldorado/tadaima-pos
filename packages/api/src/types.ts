...
export type PreSaleStatus = 'live' | 'ready' | 'completed' | 'cancelled' | 'expired' | 'paused'
...
export interface PreSale {
  ...
  customer: { id: number; name: string; tier: string | null } | null
  items: PreSaleItem[] | null
  payments: PreSalePayment[] | null
  created_at: string
  updated_at: string
}
...
export interface AddPreSalePaymentInput {
  amount: number
  payment_method_id?: number
  notes?: string
}

export interface UpdatePreSaleStatusInput {
  status: PreSaleStatus
  cancel_reason?: string
}

export interface GetPreSalesParams {
  store_id?: number
  status?: PreSaleStatus | string
  customer_id?: number
  from?: string
  to?: string
  page?: number
