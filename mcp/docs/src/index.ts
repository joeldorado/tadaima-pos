...
import { DOC_ROLES, DocBlockSchema, DocTopicSchema, TourSchema, checkTableRows } from "./schema.js"
        ...
        for (const file of CATEGORY_FILES) {
          ...
          for (const t of topics) {
            ...
            const extra = [
              ...
              .filter(Boolean)
              .join(" · ")
            lines.push(`  · ${String(t["slug"])} — ${String(t["title"])} [${cat} → data/${file}.json] (${extra})`)
          }
        }
...
server.tool(
  ...
  async ({ category_file, topic, position }) =>
    enqueue(async () => {
      try {
...
server.tool(
  ...
  async ({ slug, section, block, index }) =>
    enqueue(async () => {
      try {
        if (block.kind === "table") {
...
server.tool(
  ...
  async ({ slug, section, heading, id, blocks }) =>
    enqueue(async () => {
      try {
