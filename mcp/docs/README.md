...
`src/schema.ts` es una **copia consciente en zod** de los schemas canónicos de
landing:

- `landing/src/content/docs/schema.ts` — temas (8 kinds de bloque, tonos, roles)
- `landing/src/content/tours/schema.ts` — tours (placements, `waitFor` boolean,
  precondición con `check` de `checks.ts`)

Cualquier cambio a esos schemas **DEBE** replicarse aquí en el mismo commit —
y viceversa. Si divergen, el MCP aceptará/rechazará contenido que la UI valida
distinto.
